...
describe('InMemoryQueue', () => {
    it('delivers an event to every subscriber of that event', async () => {
        ...
        await queue.subscriber('WorkerCreated', async data => { handled.push(`first:${data.workerId}`) });
        await queue.subscriber('WorkerCreated', async data => { handled.push(`second:${data.workerId}`) });
        ...
        assert.deepEqual(handled, ['first:worker-1', 'second:worker-1']);
    });
    ...
    it('does not wait for the subscriber to finish', async () => {
        ...
        await queue.subscriber('WorkerCreated', async () => { handled = true });

        await queue.publish({ eventName: 'WorkerCreated', data: {} });
    ...
    it('does not propagate a failure of the subscriber to the publisher', async () => {
        ...
        await queue.subscriber('WorkerCreated', async () => { throw new Error('run failed') });
