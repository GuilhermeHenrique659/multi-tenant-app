...
class Step extends Subject {
    ...
    static create(agentId: string, action: string, input: any, order: number, type: StepType) {
        return new Step({
            id: Id.create(),
            agentId: new Id(agentId),
            action,
            input,
            order,
            type,
    ...
    static restore(props: { id: string; agentId: string; action: string; input: any; order: number; type: string; status: string; error?: string | null; answer?: string | null }) {
        return new Step({
            id: new Id(props.id),
            agentId: new Id(props.agentId),
            action: props.action,
            input: props.input,
            order: props.order,
