...
export default class OpenRouterLLMGateway implements LLMGateway {
    ...
    public async chat(request: LLMRequest): Promise<TupleResult<LLMResponse>> {
        ...
        // The json schema names each call (`agent_plan`, `step_input`, ...), so the logs
        // say which part of the run was talking to the provider.
        ...
        try {
            ...
            if (!response.ok) {
                ...
                Logger.error(`LLM ${call} -> ${model}: failed with ${response.status}: ${body}`);
            ...
            Logger.error(`LLM ${call} -> ${model}: transport failed: ${error.message}`);
        ...
        if (!content) {
            Logger.error(`LLM ${call} -> ${model}: answered without content`);
        ...
        Logger.info(`LLM ${call} -> ${llmResponse.model}: answered (${usage})`);
